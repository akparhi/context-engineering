...
Check `claude --version`. Use a persistent
Bun installation: setup records its executable path, so a temporary download
...
- **Models shown in `/model`** (default: curated rows for connected providers). Pass
  ...
  in the provider docs, add it to the
...
```sh
bun "${CLAUDE_PLUGIN_ROOT}/src/setup.ts"
```
...
claiming completion.
On success, tell the user to open a new terminal, run `switchboard-ctl status`, and start
the launch command (`switchboard` by default) to launch Switchboard. Unless the user
