...
import { NativeApprovalBridge } from '../../src/gateway/approval.ts';
...
import { readSse } from '../../src/providers/codex/responses.ts';
...
const initialModel = 'switchboard/openai/gpt-6-luna';
...
const policyDir = new URL('../../src/providers/codex/guardian/', import.meta.url);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    assert.equal(req.headers['x-switchboard-gateway-token'], token);
...
try {
 ...
 const response=await fetch(${JSON.stringify(base)}+endpoint,{method:'POST',headers:{'x-switchboard-gateway-token':${JSON.stringify(token)}},body:input,signal:AbortSignal.timeout(50000)});
    ...
    env: isolatedEnvironment({
      ...
      ...(launcher
        ...
        : {
            ...
            ANTHROPIC_CUSTOM_HEADERS: `x-switchboard-gateway-token: ${token}`,
          }),
