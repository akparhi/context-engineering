...
import { zenModel, zenPickerOptions } from '../../src/providers/opencode/models.ts';
...
async function runClaude(
  ...
  const child = spawn(
    ...
    ],
    {
      ...
      env: isolatedEnvironment({
        ...
        ANTHROPIC_CUSTOM_HEADERS: `x-switchboard-gateway-token: ${gatewayToken}`,
  ...
  try {
    ...
    const usage = result.modelUsage?.[`switchboard/zen/${selectedModel}`];
...
async function cancelRequest(port: number) {
  ...
  const request = fetch(`http://127.0.0.1:${port}/v1/messages`, {
    ...
    headers: { 'content-type': 'application/json', 'x-switchboard-gateway-token': gatewayToken },
    body: JSON.stringify({
