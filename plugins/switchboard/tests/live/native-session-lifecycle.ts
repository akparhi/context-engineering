...
const root = await mkdtemp(path.join(os.tmpdir(), 'switchboard-session-lifecycle-'));
const launcher = fileURLToPath(
  new URL('../../src/launcher.ts', import.meta.url),
);
const realClaude = process.env.SWITCHBOARD_REAL_CLAUDE || 'claude';
...
const env = isolatedEnvironment({
  ...
  SWITCHBOARD_REAL_CLAUDE: wrapper,
...
async function launch(session?: string) {
  ...
  assert.doesNotMatch(stdout + stderr, /DEP0190|Switchboard permission sync failed/);
...
for (const option of ['--bg', '--background', 'attach', 'respawn']) {
  await assert.rejects(
    ...
    /Switchboard sessions must stay attached/,
  );
}
console.log(
